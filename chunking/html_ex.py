...
class GraphNode:
    def __init__(self, name="", children=None, parent=None, value=None):
        self.name = name
...
def get_html_content(url_to_content="https://en.wikipedia.org/wiki/Email_marketing"):
    url = url_to_content  # Replace with the URL of the page you want to scrape
...
def extract_headers(html_content):
    ...
    for level in range(1, 7):
        ...
        headers[header_tag] = [header.get_text(strip=True) for header in soup.find_all(header_tag) if header.get_text
                               (strip=True)]
    print(headers)
    return headers
...
def print_and_return_graph(root: GraphNode):
    node_queue = [root]
    ordered_headers = []
    while node_queue:
        node = node_queue.pop(-1)
        ordered_headers.append(f"{node.name}")
        node_queue.extend(reversed(node.children))
    print(ordered_headers)
    return ordered_headers

def get_ordered_headers(chunks, headers_dict, max_level):
    ordered_header = []
    ...
    for chunk in chunks:
        ...
        for i,key in enumerate(header_keys): 
            if len(headers_dict[f"{key}"]) < 1:
                header_keys.pop(i)
            if len(headers_dict[f"{key}"]) > 0 and chunk == headers_dict[f"{key}"][0]:
                # print(header_keys)
                ordered_header.append(chunk)
                headers_dict[f"{key}"].pop(0)
            else:
                level += 1
...
def parse_and_split_html(html_content, chunk_size=100):
    ...
    chunks = [word.strip() for word in words if word not in ['\n', ' ', '']]
    return chunks
...
def build_graph(headers_content,headers):
    ...
    for header in headers_content:
        ...
        if len(header_level)==1:
            parent = root
            parent.children.append(GraphNode(name=" ".join(graph[1:]), parent=parent))
            print(" ".join(graph[1:]))
        else:
            i = 0
...
def html_split_knowledge_graph(html_content, chunk_size=100):
    ...
    ordered_headers = print_and_return_graph(root)
    print(f"ordered_headers: {ordered_headers}")

    next_header=root.children[0]
    current_header = root
    header_name = ordered_headers[0]
    h = 0
    for chunk in chunks:
        header_name = ordered_headers[h]
        if len(ordered_headers) > h+1 and chunk == ordered_headers[h+1]:
            if len(para_chunks) != 0:
                # current_header.value.append(para_chunks)
            ...
            # header_name = chunk
            h += 1
            if len(next_header.children) > 0:
                next_header = next_header.children[0]

        if chunk == paragraphs[j]:
            # print(chunk)
            para_chunks.append(chunk)
            j += 1 
            print(f"\n\n header_name {header_name} {j} {len(paragraphs)}")
            if j == len(paragraphs):
                if len(para_chunks) != 0:
                    kg_chunks[header_name] = para_chunks
...
chunks = parse_and_split_html(html_content, chunk_size=10)
headers_dict = extract_headers(html_content=html_content) 
ordered_headers = get_ordered_headers(chunks=chunks,headers_dict=headers_dict,max_level=6)
